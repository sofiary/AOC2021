...
def part_two(bingo_file: str) -> int:
    ...
    for round in range(1, bingo_rounds + 1):
        ...
        for key in list(boards.keys()):
            if check_board_state(round_numbers, boards[key]):
                # Winner
                remaining_board_count = len(boards.keys())
                if remaining_board_count == 1:
                    undrawn_sum = sum_undrawn_numbers(
                    round_numbers, list(boards.values()).pop()
                    )
                del boards[key]
